...
export default function Analytics() {
  ...
  if (loading) {
    return <div className="text-luxury-gold text-xs uppercase tracking-editorial animate-pulse">Loading Analytics...</div>;
  }
  ...
  return (
    <div className="space-y-12">
      <div className="border-b border-luxury-border pb-6">
        <h1 className="font-serif text-3xl text-white tracking-wide uppercase">Performance Intelligence</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((item, idx) => (
          <div key={idx} className="bg-luxury-charcoal border border-luxury-border/60 p-6 relative">
            <span className="text-[10px] tracking-widest text-luxury-muted uppercase block">{item.label}</span>
      ...
      <div className="bg-luxury-charcoal border border-luxury-border/60 p-8">
        <h2 className="font-serif text-xl text-white tracking-wide mb-6 uppercase">Recent Client Inquiries</h2>
            ...
            <tbody className="divide-y divide-luxury-border/40 text-white">
              {recentInquiries?.map((inquiry) => (
                <tr key={inquiry._id}>
                  ...
                  <td className="py-4 font-mono">₦ {Number(inquiry.value).toLocaleString()}</td>
                  <td className="py-4 font-mono">{new Date(inquiry.createdAt).toLocaleDateString()}</td>
